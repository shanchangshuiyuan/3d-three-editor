...
function create3dTags() {
  ...
  if (intersects.length > 0) {
      ...
      render() {
        return (
          <div>
            ...
            >
              <span className="tag-txt">{`标签-${len}`}</span>
            </div>
    ...
    const vNode = tagVNode.mount(document.createElement("div"));
...
function updateTagElement(tag) {
  ...
  if (object) {
    ...
    const { height, backgroundColor, width, fontSize, color, innerText, iconColor, iconSize, positionY, positionX, positionZ } =
      tag;
